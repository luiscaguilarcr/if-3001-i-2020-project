...
public class AdminManagePane implements PaneViewer {
    ...
    public AdminManagePane(Stage stage) {
        this.stage = stage;
    ...
    private void initializePanes(Stage stage, Pane basePane) {
        ...
        panes.put(PaneName.ADMIN_BAR, new Admin_MenuBar(stage).getPane());
    }
    ...
    private void setupBasePane(Pane pane) {
        ((GridPane) pane).add(panes.get(PaneName.ADMIN_BAR), 0, 0);
        hbox = new HBox();
